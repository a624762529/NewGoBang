                                ...
                                map<int,SockArry> &mp)
{
    ...
    case SendPackImpl::Talk:
    {
        TalkInfo *info=reinterpret_cast<TalkInfo *>(back);

        int homeid=info->m_home_id;
        int type=0;
        Home ret_home  = hall.findHome(homeid);

        Home ret_;
        ret_.cfd1=hall.findHome(homeid).talk_home.cfd1;
        ret_.cfd2=hall.findHome(homeid).talk_home.cfd2;
        int per_fd=ret_home.getPerTalkInfo(cfd);

        type= Explain::ChangeNothing;

        int flags = fcntl(per_fd,F_GETFL,0);
        flags &= ~O_NONBLOCK;
        fcntl(per_fd,F_SETFL,flags);

        write(per_fd,info,sizeof(TalkInfo));

        flags = fcntl(per_fd,F_GETFL,0);
        flags |= O_NONBLOCK;
        fcntl(per_fd,F_SETFL,flags);


        return pair<Home,int>(ret_,type);

    }
    case SendPackImpl::JoinTalkHome:
    {
        JoinTalkHome *join=reinterpret_cast<JoinTalkHome*>(back);
        int home_id=join->home_id;
        Home &ret_home= hall.findHome(home_id);
        ret_home.addTalkHome(cfd);
        return pair<Home,int>(home,Explain::ChangeNothing);
        break;
    }
    default:
